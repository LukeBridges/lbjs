	...
	window.lbjs = function(selector){
		...
		if((type == "string") && (isSizzle || isQuery)){
			if((selector.indexOf("#") == 0) && (selector.indexOf(" ") == -1)){
				...
				obj.length = 1;
			}else if((selector.indexOf(".") == 0) && (selector.indexOf(" ") == -1) && document.getElementsByClassName){
			...
			}else if((selector.indexOf("<") == 0) && document.createElement){
				var temp = document.createElement("div");
				temp.innerHTML = selector;
				var tempChildren = temp.children;
				for(i = 0, len = tempChildren.length; i < len; i += 1)
				{
